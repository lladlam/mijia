...
LEGACY_PLAINTEXT_DISABLED_MESSAGE = (
    "旧 CLI 明文认证模式已被安全策略禁用。"
    "请改用 `python -m mijiaAPI web` 启动零知识 API 服务，再通过 Bearer token + vault.bin 流程访问。"
)
...
def parse_args(args):
    ...
    parser.add_argument(
        ...
        help="旧 CLI 明文认证文件保存路径（当前已禁用，仅保留参数兼容）",
    )
    parser.add_argument(
        '--list_homes',
        action='store_true',
    ...
    run.add_argument(
        ...
        type=Path,
        default=get_default_auth_path(),
        help="旧 CLI 明文认证文件保存路径，Windows 默认保存在当前目录 .mijia-api-legacy/auth.json",
    )
    run.add_argument(
        'prompt',
        type=str,
    ...
    web = subparsers.add_parser(
        'web',
        help="启动纯 API 服务（不再提供网页端）",
    )
    web.set_defaults(func='web')
    web.add_argument(
        '--state_dir',
        type=Path,
        default=get_default_auth_path().parent.parent / ".mijia-server",
        help="服务状态目录，按 token 隔离保存每个用户的 vault.bin、会话元数据与设备缓存",
    )
    web.add_argument(
        '--host',
        type=str,
        default='127.0.0.1',
        help="本地Web服务监听地址，默认 127.0.0.1",
    )
    web.add_argument(
        '--port',
        type=int,
        default=8123,
        help="本地Web服务端口，默认 8123",
    )
    web.add_argument(
        '--production',
        action='store_true',
        help="兼容保留参数，API-only 服务默认按生产安全方式运行",
    )
    web.add_argument(
        '--workers',
        type=int,
        default=1,
        help="服务 worker 数量，出于会话一致性安全限制当前仅支持 1",
    )
    web.add_argument(
        '--max_concurrency',
        type=int,
        default=1000,
        help="服务允许的最大并发请求数，默认 1000",
    )
    ...
    logout.set_defaults(func='logout')
    logout.add_argument(
        '-p', '--auth_path',
        type=Path,
        default=get_default_auth_path(),
...
def init_api(auth_path: Path) -> mijiaAPI:
    raise RuntimeError(LEGACY_PLAINTEXT_DISABLED_MESSAGE)
...
def logout(args):
    ...
    default_auth_path = get_default_auth_path().resolve(strict=False)
    target_auth_path = auth_path.resolve(strict=False)
    if target_auth_path != default_auth_path or target_auth_path.name != "auth.json":
        raise RuntimeError(f"出于安全原因，仅允许删除默认认证文件: {default_auth_path}")
    target_auth_path.unlink(missing_ok=True)
    print(f"已清除认证文件: {target_auth_path}")
...
def main(args):
    ...
    if hasattr(args, 'func') and args.func == 'web':
        ...
        return
    if hasattr(args, 'func') and args.func == 'logout':
        logout(args)
        return
