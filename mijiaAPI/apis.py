...
def redact_auth_data(auth_data: dict) -> dict:
    redacted = {}
    for key, value in auth_data.items():
        if key in SENSITIVE_AUTH_KEYS:
...
def _summarize_response_payload(payload: dict) -> dict:
    summary = {
    ...
    if isinstance(payload.get("result"), dict):
        ...
        summary["result_length"] = len(payload["result"])
    return summary


def get_default_auth_path() -> Path:
    auth_path_override = os.getenv("MIJIA_AUTH_PATH")
    if auth_path_override:
        return Path(auth_path_override)
    if os.name == "nt":
        return Path.cwd() / ".mijia-api-legacy" / "auth.json"
    return Path.home() / ".config" / "mijia-api-legacy" / "auth.json"


class mijiaAPI():
    def __init__(
            self,
            auth_data_path: Optional[str] = None,
            auth_data: Optional[dict] = None,
            auth_data_save_hook: Optional[Callable[[dict], None]] = None,
            allow_plaintext_persistence: bool = False,
        ...
        if auth_data_path is None:
            self.auth_data_path = get_default_auth_path()
        ...
        self._auth_data_save_hook = auth_data_save_hook
        self.allow_plaintext_persistence = allow_plaintext_persistence

        if auth_data is not None:
            self.auth_data = dict(auth_data)
            if self.auth_data:
                self._init_session()
    ...
    def _request_with_retry(
            ...
            *args,
            ...
            timeout_message: str = "请求米家服务器超时",
            error_message: str = "请求米家服务器失败",
            **kwargs,
    ) -> requests.Response:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                return requester(*args, timeout=timeout, **kwargs)
            except requests.exceptions.Timeout as exc:
                last_error = exc
                logger.warning("%s，第 %s 次重试", timeout_message, attempt + 1)
                if attempt >= self.max_retries:
                    raise LoginError(-10006, timeout_message)
            except requests.exceptions.RequestException as exc:
                last_error = exc
                logger.warning("%s，第 %s 次重试: %s", error_message, attempt + 1, exc)
                if attempt >= self.max_retries:
                    raise LoginError(-10001, f"{error_message}: {exc}")
            time.sleep(self.retry_delay)
        raise LoginError(-10001, f"{error_message}: {last_error}")
    ...
    def _handle_ret(self, fetch_ret: requests.Response, verify_code: bool = True) -> dict:
        if fetch_ret.status_code != 200:
            logger.warning("上游登录接口返回非 200 状态码: %s", fetch_ret.status_code)
            raise LoginError(fetch_ret.status_code, "上游登录接口返回异常状态码")
    ...
    def _save_auth_data(self):
        ...
        if self._auth_data_save_hook is not None:
            self._auth_data_save_hook(dict(self.auth_data))
            return
        if not self.allow_plaintext_persistence:
            logger.debug("认证数据已更新，仅保留内存，未写入明文 auth.json")
            logger.debug("认证数据(脱敏): %s", redact_auth_data(self.auth_data))
            return
    ...
    def _get_location(self) -> dict:
        ...
        service_ret = self._request_with_retry(
            requests.get,
            self.service_login_url,
            headers=headers,
            timeout_message="请求米家登录服务器超时",
            error_message="请求米家登录服务器失败",
        )
        service_data = self._handle_ret(service_ret, verify_code=False)
        location = service_data["location"]
        if service_data['code'] == 0:
            ret = self._request_with_retry(
                self.session.get,
                location,
                timeout_message="刷新 token 超时",
                error_message="刷新 token 失败",
            )
    ...
    def QRlogin(self) -> dict:
        ...
        login_ret = self._request_with_retry(
            requests.get,
            url,
            headers=headers,
            timeout_message="获取扫码登录二维码超时",
            error_message="获取扫码登录二维码失败",
        )
        ...
        try:
            lp_ret = self._request_with_retry(
                session.get,
                login_data["lp"],
                headers=headers,
                timeout=self.login_timeout,
                timeout_message="扫码登录超时，请重试",
        ...
        self._request_with_retry(
            ...
            callback_url,
            headers=headers,
            timeout_message="登录回调超时",
            error_message="登录回调失败",
        )
    ...
    def _request(self, uri: str, data: dict, refresh_token: bool = True) -> dict:
        logger.debug("请求 URI: %s，摘要: %s", uri, _summarize_payload(data))
        if refresh_token:
            self._refresh_token()
        ...
        ret = self._request_with_retry(
            ...
            url,
            data=params,
            timeout_message="请求米家接口超时",
            error_message="请求米家接口失败",
        )
        try:
        ...
        logger.debug("响应摘要: uri=%s, %s", uri, _summarize_response_payload(ret_data))
    ...
    def get_statistics(self, data: dict) -> list:
        ...
        返回值：
            list: 统计数据列表，每项包含以下字段：
                - value (str): 统计值（通常可用 ast.literal_eval() 安全解析）
                - time (int): 时间戳
        ...
        示例：
            ...
            >>> for item in ret:
            ...     import ast
            ...     value = ast.literal_eval(item['value'])[0]
            ...     ts = item['time']
